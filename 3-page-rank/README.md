...
$$
换句话说，算法将根据每个页面连出总数$$L(x)$$（**亦即图中节点的出度**）平分该页面的PR值，并将其加到其所指向的页面：
$$
...
\end{pmatrix}d+\frac{1-d}{N}
$$
所以，对于某个页面i，其对应PR值大小的计算公式如下：
$$
PR(p_{i})=\frac{1-d}{N}+d\sum_{p_{j}\in Q(p_{i})}\frac{PR(p_{j})}{L(p_{j})}
$$
其中$$Q(p_{i})$$是链入页面$$p_{i}$$的集合。
...
\end{bmatrix} \\
\\
其中l(p_{i},p_{j}) =\left\{\begin{matrix}
\frac{1}{L(j)}, 页面j有指向页面i的链接 \\
0，页面j没有指向页面i的链接
\end{matrix}\right.，\\
L(j)是页面j的链出总数（亦即图中节点的出度）\\
则R' = \begin{bmatrix}
PR(p_{1}) \\
PR(p_{2}) \\
\vdots \\
PR(p_{N}) \\
\end{bmatrix} = \frac{1-d}{n}1+dMR
$$
容易证明，上述迭代式中R是收敛的。

## 任务1：迭代算法

将每个点的初始PR值均设置为$$\frac{1}{N}$$。设定一个精确度$$ε$$，迭代计算$$R'=\frac{1-d}{n}1+dMR$$，直到$$|R’-R|≤ε$$即可。实践中，几次迭代后$$R'$$即可在极高的精确度下估计$$R$$的值。
